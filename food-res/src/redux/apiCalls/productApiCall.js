...
export function fetchProducts() {
  return async (dispatch) => {
    try {
      ...
      const response = await fetch(`https://backend-resturant-food-1.onrender.com/uploads/products`);
...
export function fetchProductById(byId){
  return async (dispatch) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URI}/uploads/products/${byId}`);
      const data = await response.json();
